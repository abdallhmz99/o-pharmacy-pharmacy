...
editPhonesForm = new FormGroup({
  'phones': new FormControl(null, [Validators.required, Validators.maxLength(12),Validators.min(3)])
    });
...
isLogged(){
  if(this.token){
    ...
    this.phone=this.decoded.phones;
...
{
  if(this.url!=null){
    this._EditProfileService.editPharmacyLogo(this.url).subscribe(d => {
      this.logoMessage=d.message;
      console.log(d)
    },
      err => {
        console.log(err);
